...
contract MyRelayDeploy is RelayDeploy {
    ...
    uint48 internal immutable EPOCH_DURATION = 1 hours;
    ...
    uint256 internal immutable OPERATOR_COUNT = 10;
    ...
    function getNetwork(bool deploy) internal returns (address) {
        if (deploy && config.get("network").data.length == 0) {
            ...
            (
                IVotingPowerProvider.VotingPowerProviderInitParams({
                    networkManagerInitParams: INetworkManager.NetworkManagerInitParams({
                        network: getNetwork(true), subnetworkId: 0
                    }),
        ...
        initData = abi.encodeCall(
            ...
            (
                ISettlement.SettlementInitParams({
                    networkManagerInitParams: INetworkManager.NetworkManagerInitParams({
                        network: getNetwork(false), subnetworkId: 0
                    }),
    ...
    function runDeployKeyRegistry() public override {
        deployKeyRegistry({proxyOwner: getDeployerAddress(), isDeployerGuarded: true, salt: KEY_REGISTRY_SALT});

        fundOperators(0.03 ether);
    ...
    function runDeployVotingPowerProvider() public override {
        address votingPowerProvider = deployVotingPowerProvider({
            proxyOwner: getDeployerAddress(), isDeployerGuarded: true, salt: VOTING_POWER_PROVIDER_SALT
        });
        address network = getNetwork(true);
        SymbioticCoreConstants.Core memory core = getCore();
    ...
    function runDeploySettlement() public override {
        address settlement =
            deploySettlement({proxyOwner: getDeployerAddress(), isDeployerGuarded: true, salt: SETTLEMENT_SALT});
        vm.broadcast();
        address sumTask = deployCreate3WithGuardedSalt(
            getDeployerAddress(), SUM_TASK_SALT, abi.encodePacked(type(SumTask).creationCode, abi.encode(settlement))
        );
        config.set("sum_task", sumTask);

        fundOperators(0.001 ether);
    }
    ...
    function registerOperator(uint256 index, uint256 stakeAmount) public {
        ...
        getCore().operatorNetworkOptInService.optIn(address(getNetwork(false)));
    ...
    function printOperatorsInfo() public {
        ...
        logMessage = string.concat(logMessage, "Operator private keys:\n");
        for (uint256 i; i < OPERATOR_COUNT; ++i) {
            Vm.Wallet memory operator = getOperator(i);
            logMessage = string.concat(logMessage, "   Address: ", vm.toString(operator.addr), "\n");
            logMessage = string.concat(logMessage, "   Private key: ", vm.toString(operator.privateKey), "\n");
        }
